...
internal class Day8 : ISolution
{
    ...
    private readonly List<HashSet<Position>> _circuits = [];
    ...
    public void Part2()
    {
        ...
        foreach (var (pos1, pos2) in combinations)
        {
            ...
            if (_circuits.Select(x => x.Count).Max() == positions.Length)
            {
    ...
    private void CombineCircuits(Position pos1, Position pos2)
    {
        var pos1Circuit = _circuits.SingleOrDefault(x => x.Contains(pos1));
        var pos2Circuit = _circuits.SingleOrDefault(x => x.Contains(pos2));
